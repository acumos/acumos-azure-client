...
public final class Utils {
	...
	public static void print(VirtualMachine resource) {
		...
		msi.append("\n\t\t\tMSI Active Directory Service Principal Id:")
		...
		log.debug(new StringBuilder().append("Virtual Machine: ").append(resource.id()).append("Name: ")
				...
				.append(osProfile).append(networkProfile).append(extensions).append(msi).toString());
	}
	 ...
	 *
	 * @param storageAccountKeys
	 *            a list of storage account keys
	 */
	 ...
	 *
	 * @param redisAccessKeys
	 *            a keys for Redis Cache
	 */
	...
	public static void print(LoadBalancer resource) {
		...
		for (LoadBalancerInboundNatRule natRule : resource.inboundNatRules().values()) {
			info.append("\n\t\tInbound NAT rule name: ").append(natRule.name()).append("\n\t\t\tProtocol: ")
					...
					.append(natRule.backendNetworkInterfaceId()).append("\n\t\t\tBackend NIC IP config name: ")
					.append(natRule.backendNicIPConfigurationName()).append("\n\t\t\tFloating IP? ")
					.append(natRule.floatingIPEnabled()).append("\n\t\t\tIdle timeout in minutes: ")
	...
	public static void print(BatchAccount batchAccount) {
		...
		if (batchAccount.applications().size() > 0) {
			for (Map.Entry<String, Application> applicationEntry : batchAccount.applications().entrySet()) {
				...
				for (Map.Entry<String, ApplicationPackage> applicationPackageEntry : application.applicationPackages()
						.entrySet()) {
		...
		log.debug(new StringBuilder().append("BatchAccount: ").append(batchAccount.id()).append("Name: ")
				...
				.append(batchAccount.poolQuota()).append("\n\tActiveJobAndJobScheduleQuota: ")
	 ...
	 *
	 * @param resource
	 *            an app service certificate order
	 */
	...
	public static void print(WebAppBase resource) {
		StringBuilder builder = new StringBuilder().append("Web app: ").append(resource.id()).append("Name: ")
				...
				.append(resource.appServicePlanId()).append("\n\tHost name bindings: ");
		...
		if (!azureEndpoints.isEmpty()) {
			...
			for (TrafficManagerAzureEndpoint endpoint : azureEndpoints.values()) {
				info.append("\n\t\tAzure endpoint: #").append(idx++).append("\n\t\t\tId: ").append(endpoint.id())
						...
						.append(endpoint.targetAzureResourceId()).append("\n\t\t\tTarget resourceType: ")
						.append(endpoint.targetResourceType()).append("\n\t\t\tMonitor status: ")
		...
		if (!nestedProfileEndpoints.isEmpty()) {
			...
			for (TrafficManagerNestedProfileEndpoint endpoint : nestedProfileEndpoints.values()) {
				info.append("\n\t\tNested profile endpoint: #").append(idx++).append("\n\t\t\tId: ")
						...
						.append(endpoint.isEnabled()).append("\n\t\t\tRouting priority: ")
						.append(endpoint.routingPriority()).append("\n\t\t\tRouting weight: ")
	...
	public static void print(DnsZone dnsZone) {
		...
		info.append("DNS Zone: ").append(dnsZone.id()).append("\n\tName (Top level domain): ").append(dnsZone.name())
				...
				.append(dnsZone.regionName()).append("\n\tTags: ").append(dnsZone.tags()).append("\n\tName servers:");
	...
	public static void print(ContainerService containerService) {
		...
		info.append("Azure Container Service: ").append(containerService.id()).append("\n\tName: ")
				.append(containerService.name()).append("\n\tWith orchestration: ")
				.append(containerService.orchestratorType().toString()).append("\n\tMaster FQDN: ")
				.append(containerService.masterFqdn()).append("\n\tMaster node count: ")
				.append(containerService.masterNodeCount()).append("\n\tMaster leaf domain label: ")
				.append(containerService.masterLeafDomainLabel()).append("\n\t\tWith Agent pool name: ")
				.append(containerService.agentPoolName()).append("\n\t\tAgent pool count: ")
				.append(containerService.agentPoolCount()).append("\n\t\tAgent pool count: ")
				.append(containerService.agentPoolVMSize().toString()).append("\n\t\tAgent pool FQDN: ")
				.append(containerService.agentPoolFqdn()).append("\n\t\tAgent pool leaf domain label: ")
				.append(containerService.agentPoolLeafDomainLabel()).append("\n\tLinux user name: ")
				.append(containerService.linuxRootUsername()).append("\n\tSSH key: ").append(containerService.sshKey());
	 ...
	 * 
	 * @param envSecondaryServicePrincipal
	 *            an Azure Container Registry
	...
	/**
	 * Creates and returns a randomized name based on the prefix file for use by the
	 * sample.
	 * 
	 * @param namePrefix
	 *            The prefix string to be used in generating the name.
	...
	public static void createCertificate(String certPath, String pfxPath, String alias, String password, String cnName)
		...
		String[] commandArgs = { command, "-genkey", "-alias", alias, "-keystore", pfxPath, "-storepass", password,
				"-validity", validityInDays, "-keyalg", keyAlg, "-sigalg", sigAlg, "-keysize", keySize, "-storetype",
				storeType, "-dname", "CN=" + cnName, "-ext", "EKU=1.3.6.1.5.5.7.3.1" };
		...
		if (pfxFile.exists()) {
			...
			throw new IOException("Error occurred while creating certificates" + StringUtils.join(" ", commandArgs));
		}
	}
	 ...
	 *            : Boolean which controls whether to throw exception or not based
	 *            on error stream.
	 * @return result :- depending on the method invocation.
	 * @throws Exception
	 *             exceptions thrown from the execution
	 */
	 ...
	 * 
	 * @param elasticPoolActivity
	 *            elastic pool activity to be printed
	 */
	public static void print(ElasticPoolActivity elasticPoolActivity) {
		StringBuilder builder = new StringBuilder().append("Sql elastic pool activity: ")
				...
				.append(elasticPoolActivity.state()).append("\n\tElastic pool name: ")
	 ...
	 * 
	 * @param databaseActivity
	 *            database activity to be printed
	 */
	...
	public static void print(ApplicationGateway resource) {
		...
		info.append("Application gateway: ").append(resource.id()).append("Name: ").append(resource.name())
				...
				.append(resource.privateIPAllocationMethod()).append("\n\tDisabled SSL protocols: ")
		...
		for (ApplicationGatewayProbe probe : probes.values()) {
			info.append("\n\t\tName: ").append(probe.name()).append("\n\t\tProtocol:")
					.append(probe.protocol().toString()).append("\n\t\tInterval in seconds: ")
					.append(probe.timeBetweenProbesInSeconds()).append("\n\t\tRetries: ")
					.append(probe.retriesBeforeUnhealthy()).append("\n\t\tTimeout: ").append(probe.timeoutInSeconds())
		...
		for (ApplicationGatewayRequestRoutingRule rule : rules.values()) {
			info.append("\n\t\tName: ").append(rule.name()).append("\n\t\t\tType: ").append(rule.ruleType())
					...
					.append(rule.frontendProtocol().toString()).append("\n\t\t\tBackend port: ")
					.append(rule.backendPort()).append("\n\t\t\tCookie based affinity enabled? ")
	 ...
	 * 
	 * @param profile
	 *            the publishing profile for the web app.
	 * @param fileName
	 *            the name of the file on server
	 * @param file
	 *            the local file
	 */
	...
	public static void print(ServiceBusNamespace serviceBusNamespace) {
		StringBuilder builder = new StringBuilder().append("Service bus Namespace: ").append(serviceBusNamespace.id())
				.append("\n\tName: ").append(serviceBusNamespace.name()).append("\n\tRegion: ")
				.append(serviceBusNamespace.regionName()).append("\n\tResourceGroupName: ")
				.append(serviceBusNamespace.resourceGroupName()).append("\n\tCreatedAt: ")
				.append(serviceBusNamespace.createdAt()).append("\n\tUpdatedAt: ")
				.append(serviceBusNamespace.updatedAt()).append("\n\tDnsLabel: ").append(serviceBusNamespace.dnsLabel())
				.append("\n\tFQDN: ").append(serviceBusNamespace.fqdn()).append("\n\tSku: ").append("\n\t\tCapacity: ")
				.append(serviceBusNamespace.sku().capacity()).append("\n\t\tSkuName: ")
	...
	public static void print(Queue queue) {
		StringBuilder builder = new StringBuilder().append("Service bus Queue: ").append(queue.id())
				...
				.append(queue.duplicateMessageDetectionHistoryDuration()).append("\n\tIsBatchedOperationsEnabled: ")
				.append(queue.isBatchedOperationsEnabled()).append("\n\tIsDeadLetteringEnabledForExpiredMessages: ")
				.append(queue.isDeadLetteringEnabledForExpiredMessages()).append("\n\tIsDuplicateDetectionEnabled: ")
				.append(queue.isDuplicateDetectionEnabled()).append("\n\tIsExpressEnabled: ")
				.append(queue.isExpressEnabled()).append("\n\tIsPartitioningEnabled: ")
				.append(queue.isPartitioningEnabled()).append("\n\tIsSessionEnabled: ").append(queue.isSessionEnabled())
				.append("\n\tDeleteOnIdleDurationInMinutes: ").append(queue.deleteOnIdleDurationInMinutes())
				.append("\n\tMaxDeliveryCountBeforeDeadLetteringMessage: ")
				.append(queue.maxDeliveryCountBeforeDeadLetteringMessage()).append("\n\tMaxSizeInMB: ")
				.append(queue.maxSizeInMB()).append("\n\tMessageCount: ").append(queue.messageCount())
				.append("\n\tScheduledMessageCount: ").append(queue.scheduledMessageCount()).append("\n\tStatus: ")
				.append(queue.status()).append("\n\tTransferMessageCount: ").append(queue.transferMessageCount())
	 ...
	 * 
	 * @param queueAuthorizationRule
	 *            a service bus queue authorization keys
	 */
	public static void print(QueueAuthorizationRule queueAuthorizationRule) {
		StringBuilder builder = new StringBuilder().append("Service bus queue authorization rule: ")
				.append(queueAuthorizationRule.id()).append("\n\tName: ").append(queueAuthorizationRule.name())
	 ...
	 * 
	 * @param keys
	 *            a service bus namespace authorization keys
	 */
	public static void print(AuthorizationKeys keys) {
		StringBuilder builder = new StringBuilder().append("Authorization keys: ").append("\n\tPrimaryKey: ")
				.append(keys.primaryKey()).append("\n\tPrimaryConnectionString: ")
	...
	public static void print(Topic topic) {
		StringBuilder builder = new StringBuilder().append("Service bus topic: ").append(topic.id())
				...
				.append("\n\tDuplicateMessageDetectionHistoryDuration: ")
				.append(topic.duplicateMessageDetectionHistoryDuration()).append("\n\tIsBatchedOperationsEnabled: ")
				.append(topic.isBatchedOperationsEnabled()).append("\n\tIsDuplicateDetectionEnabled: ")
				.append(topic.isDuplicateDetectionEnabled()).append("\n\tIsExpressEnabled: ")
				.append(topic.isExpressEnabled()).append("\n\tIsPartitioningEnabled: ")
				.append(topic.isPartitioningEnabled()).append("\n\tDeleteOnIdleDurationInMinutes: ")
				.append(topic.deleteOnIdleDurationInMinutes()).append("\n\tMaxSizeInMB: ").append(topic.maxSizeInMB())
	...
	public static void print(ServiceBusSubscription serviceBusSubscription) {
		StringBuilder builder = new StringBuilder().append("Service bus subscription: ")
				.append(serviceBusSubscription.id()).append("\n\tName: ").append(serviceBusSubscription.name())
				...
				.append(serviceBusSubscription.updatedAt()).append("\n\tAccessedAt: ")
				.append(serviceBusSubscription.accessedAt()).append("\n\tActiveMessageCount: ")
				.append(serviceBusSubscription.activeMessageCount()).append("\n\tDeadLetterMessageCount: ")
				.append(serviceBusSubscription.deadLetterMessageCount()).append("\n\tDefaultMessageTtlDuration: ")
				.append(serviceBusSubscription.defaultMessageTtlDuration()).append("\n\tIsBatchedOperationsEnabled: ")
				.append(serviceBusSubscription.isBatchedOperationsEnabled())
				.append("\n\tDeleteOnIdleDurationInMinutes: ")
				.append(serviceBusSubscription.deleteOnIdleDurationInMinutes()).append("\n\tScheduledMessageCount: ")
				.append(serviceBusSubscription.scheduledMessageCount()).append("\n\tStatus: ")
				.append(serviceBusSubscription.status()).append("\n\tTransferMessageCount: ")
	 ...
	 * 
	 * @param topicAuthorizationRule
	 *            a topic Authorization Rule
	 */
	public static void print(TopicAuthorizationRule topicAuthorizationRule) {
		StringBuilder builder = new StringBuilder().append("Service bus topic authorization rule: ")
				.append(topicAuthorizationRule.id()).append("\n\tName: ").append(topicAuthorizationRule.name())
	 ...
	 * Print CosmosDB info.
	 * 
	 * @param cosmosDBAccount
	 *            a CosmosDB
	 */
	public static void print(CosmosDBAccount cosmosDBAccount) {
		StringBuilder builder = new StringBuilder().append("CosmosDB: ").append(cosmosDBAccount.id())
				.append("\n\tName: ").append(cosmosDBAccount.name()).append("\n\tResourceGroupName: ")
				.append(cosmosDBAccount.resourceGroupName()).append("\n\tKind: ")
				.append(cosmosDBAccount.kind().toString()).append("\n\tDefault consistency level: ")
				.append(cosmosDBAccount.consistencyPolicy().defaultConsistencyLevel()).append("\n\tIP range filter: ")
	...
	public static void print(ActiveDirectoryUser user) {
		StringBuilder builder = new StringBuilder().append("Active Directory User: ").append(user.id())
	...
	public static void print(RoleDefinition role) {
		StringBuilder builder = new StringBuilder().append("Role Definition: ").append(role.id()).append("\n\tName: ")
				.append(role.name()).append("\n\tRole Name: ").append(role.roleName()).append("\n\tType: ")
				.append(role.type()).append("\n\tDescription: ").append(role.description()).append("\n\tType: ")
				.append(role.type());
	...
	public static void print(RoleAssignment roleAssignment) {
		StringBuilder builder = new StringBuilder().append("Role Assignment: ").append("\n\tScope: ")
	...
	public static void print(ActiveDirectoryGroup group) {
		StringBuilder builder = new StringBuilder().append("Active Directory Group: ").append(group.id())
		...
		for (ActiveDirectoryObject object : group.listMembers()) {
			builder.append("\n\t\tType: ").append(object.getClass().getSimpleName()).append("\tName: ")
					.append(object.name());
		}
	...
	public static void print(ActiveDirectoryApplication application) {
		StringBuilder builder = new StringBuilder().append("Active Directory Application: ").append(application.id())
				.append("\n\tName: ").append(application.name()).append("\n\tSign on URL: ")
				.append(application.signOnUrl()).append("\n\tReply URLs:");
	...
	public static void print(ServicePrincipal servicePrincipal) {
		StringBuilder builder = new StringBuilder().append("Service Principal: ").append(servicePrincipal.id())
	...
	public static void print(PacketCapture resource) {
		StringBuilder sb = new StringBuilder().append("Packet Capture: ").append(resource.id()).append("\n\tName: ")
	...
	public static void print(Topology resource) {
		StringBuilder sb = new StringBuilder().append("Topology: ").append(resource.id()).append("\n\tResource group: ")
		...
		for (TopologyResource tr : resource.resources().values()) {
			...
			for (TopologyAssociation association : tr.associations()) {
				sb.append("\n\t\t\tName:").append(association.name()).append("\n\t\t\tResource id:")
						.append(association.resourceId()).append("\n\t\t\tAssociation type:")
	...
	public static void print(FlowLogSettings resource) {
		log.debug(new StringBuilder().append("Flow log settings: ").append("Target resource id: ")
				...
				.append("\n\tRetention policy days: ").append(resource.retentionDays()).toString());
	}
	...
	public static void print(SecurityGroupView resource) {
		StringBuilder sb = new StringBuilder().append("Security group view: ").append("\n\tVirtual machine id: ")
