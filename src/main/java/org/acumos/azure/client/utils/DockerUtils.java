...
public class DockerUtils {
    ...
    public static DockerClient createDockerClient(Azure azure, String rgName, Region region,
                                                  String registryServerUrl, String username, String password, String localEnvDockerHost, String localEnvDockerCertPath,
        ...
        log.info("====createDockerClient====networkSecurityGroup======"+networkSecurityGroup+"====dockerRegistryPort======="+dockerRegistryPort+"===dockerPort="+dockerPort);
        if (envDockerHost == null || envDockerHost.isEmpty()) {
            ...
            dockerClient = fromNewDockerVM(azure, rgName, region, registryServerUrl, username, password,azureBean,networkSecurityGroup,dockerRegistryPort);
        } else {
    ...
    public static DockerClient fromNewDockerVM(Azure azure, String rgName, Region region,
                                               String registryServerUrl, String username, String password,AzureBean azureBean,String networkSecurityGroup,String dockerRegistryPort) throws Exception {
        ...
        log.info("========networkSecurityGroup======"+networkSecurityGroup+"====dockerRegistryPort======="+dockerRegistryPort);
        // Could not find a Docker environment; presume that there is no local Docker engine running and
        //    attempt to configure a Docker engine running inside a new    Azure virtual machine
        ...
        /*Network network = azure.networks().define(vnetName)
                ...
                .withAddressSpace("135.197.0.0/16")
                .defineSubnet(subNet)
                    .withAddressPrefix("135.197.0.0/16")
                    .attach()
                    .create();*/
        ...
        NetworkSecurityGroup frontEndNSG = azure.networkSecurityGroups().getByResourceGroup(rgName, networkSecurityGroup);
       ...
       /* VirtualMachine dockerVM = azure.virtualMachines().define(dockerVMName)
        ...
        VirtualMachine dockerVM = azure.virtualMachines().define(frontEndNSGName)
                .withRegion(region)
                .withExistingResourceGroup(rgName)
        ...
        log.info( azure.publicIPAddresses().list()+"====================Created VirtualMachine====1===NetworkSecurityGroup====="+dockerVM.getPrimaryNetworkInterface().getNetworkSecurityGroup().id());
        log.info("====================Created VirtualMachine===2====NetworkSecurityGroup====="+dockerVM.getPrimaryNetworkInterface().getNetworkSecurityGroup().key());
        log.info("====================Created VirtualMachine====3===NetworkSecurityGroup====="+dockerVM.getPrimaryNetworkInterface().getNetworkSecurityGroup().name());
        log.info("====================Created VirtualMachine=====4==NetworkSecurityGroup====="+dockerVM.getPrimaryNetworkInterface().getNetworkSecurityGroup().resourceGroupName());
        log.info("Walking through network security groups");
        
        Date t2 = new Date();
        System.out.println("Created Azure Virtual Machine: (took " + ((t2.getTime() - t1.getTime()) / 1000) + " seconds) " + dockerVM.id() + " , " + dockerVM.toString());
